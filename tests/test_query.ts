...
describe('test Query methods', () => {
  ...
  describe('checkRequired', () => {
    ...
    it('will throw an error when a required property is missing from queryObject', () => {
      queryObject = { age: 25 };
      assertThrows(() => { query.checkRequired(queryObject, propertyName, propertyOptions) });
    })
    it('will return true when required properties are in queryObject', ()=> {
      queryObject = { name: "Mr. C" };
      assertStrictEquals(query.checkRequired(queryObject, propertyName, propertyOptions), true);
    })
  ...
  describe('populateQuery', () => {
    ...
    it('will assign converted value (STRING) to updatedQueryObject', () => {
      ...
      propertyName = 'name';
      propertyOptions = testSchema.schemaMap[propertyName];
      query.populateQuery(queryObject, propertyName, propertyOptions, updatedQueryObject);
      assertStrictEquals(updatedQueryObject[propertyName], queryObject[propertyName]);
    })
  ...
  describe('formatQueryString result', () => {
    ...
    it('will return propertyName when embeddedUnique is empty', () => {
      assertStrictEquals(query.formatQueryField(propertyName, []), propertyName);
    })
    it('will return concatenated string using elements from embeddedUniqeProperty and propertyName', () => {
      assertStrictEquals(query.formatQueryField(propertyName, propArray), `address.property_id.${propertyName}`);
    })
  }) 
